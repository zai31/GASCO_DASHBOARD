...
from Modules.maintenance import (
    solve_true_min_cost_mip,
    solve_true_min_cost_and_min_gap,
    solve_true_min_cost_and_max_gap,
)

# Import compressor data functions
try:
    from Modules.compressor_data import (
...
def detect_year_columns(df):
    ...
    for col in df.columns:
        match = re.match(r"(\d{4}) Budget", col)
        if match:
            year = match.group(1)
            budget_cols[year] = col

        match = re.match(r"(\d{4}) Consumed", col)
        if match:
            year = match.group(1)
...
BUDGET_COLUMNS = {"2023": "2023 Budget", "2024": "2024 Budget", "2025": "2025 Budget"}
...
# 2. FIX: Improve load_budget_data function with better error handling
def load_budget_data():
    try:
        ...
        budget_cols = {col.split()[0]: col for col in df.columns if 'Budget' in col and col.split()[0].isdigit()}
        consumed_cols = {col.split()[0]: col for col in df.columns if 'Consumed' in col and col.split()[0].isdigit()}
        ...
        if AVAILABLE_COL not in df.columns:
            current_year_budget = f"{datetime.now().year} Budget"
            if current_year_budget in df.columns:
                df[AVAILABLE_COL] = df[current_year_budget] - df[CONSUMED_COL]
            else:
                df[AVAILABLE_COL] = 0
        ...
        if "Date" in df.columns:
            ...
            # Fill NaT dates with current date
        ...
        df = df.dropna(subset=["Cost Center Name", "Account name"])
    ...
    try:
        ...
        if "Cost Center Number" in df.columns and "Cost Center Name" in df.columns:
            df["Cost Center Display"] = (
                df["Cost Center Number"].astype(str) + " - " + df["Cost Center Name"]
            )

        # Combined account for dropdown
        if "Account number" in df.columns and "Account name" in df.columns:
            df["Account Display"] = (
                df["Account number"].astype(str) + " - " + df["Account name"]
            )

        # Extract unique values for the expected return
        cost_center_names = (
            sorted(df["Cost Center Name"].dropna().unique().tolist())
            if "Cost Center Name" in df.columns
            else []
        )
        cost_center_numbers = (
            sorted(df["Cost Center Number"].dropna().unique().tolist())
            if "Cost Center Number" in df.columns
            else []
        )
        account_names = (
            sorted(df["Account name"].dropna().unique().tolist())
            if "Account name" in df.columns
            else []
        )
        account_numbers = (
            sorted(df["Account number"].dropna().unique().tolist())
...
def log_expense_form():
    ...
    with st.form("log_expense_form"):
        ...
        selected_cc_display = st.selectbox(
            "Cost Center", sorted(df["Cost Center Display"].unique())
        )
        ...
        filtered_acc_displays = (
            df[df["Cost Center Display"] == selected_cc_display]["Account Display"]
            .dropna()
            .unique()
            .tolist()
        )
        selected_acc_display = st.selectbox(
            "Select Account", sorted(filtered_acc_displays)
        )
...
def show_filtered_dashboard():
    ...
    # ------------------ 📥 Load Data First ------------------
    df, cost_center_names, cost_center_numbers, account_names, account_numbers = (
        load_budget_data()
    )

    BUDGET_COLUMNS, CONSUMED_COLUMNS = detect_year_columns(df)
        ...
        if "Cost Center Display" in df.columns and not df.empty:
            ...
            if cost_center_options:
                ...
                if not cc_rows.empty:
                    ...
                    filtered_acc_displays = (
                        cc_rows["Account Display"].dropna().unique().tolist()
                    )
                    if filtered_acc_displays:
                        with st.form("log_expense_form_main"):
                            ...
                            acc_row = cc_rows[
                                cc_rows["Account Display"] == selected_acc_display
                            ].iloc[0]
                            selected_acc_number = acc_row["Account number"]
                            selected_acc_name = acc_row["Account name"]
                            expense_date = st.date_input(
                                "📅 Expense Date", value=datetime.now()
                            )
                            match = df[
                                (df["Cost Center Number"] == selected_cc_number)
                                & (df["Cost Center Name"] == selected_cc_name)
                            ...
                            if not match.empty:
                                budget_2025 = (
                                    match["2025 Budget"].iloc[0]
                                    if "2025 Budget" in match.columns
                                    else 0
                                )
                                budget_2024 = (
                                    match["2024 Budget"].iloc[0]
                                    if "2024 Budget" in match.columns
                                    else 0
                                )
                                budget_2023 = (
                                    match["2023 Budget"].iloc[0]
                                    if "2023 Budget" in match.columns
                                    else 0
                                )
                                consumed_before = (
                                    match[CONSUMED_COL].iloc[0]
                                    if CONSUMED_COL in match.columns
                                    else 0
                                )
                                available_before = (
                                    match[AVAILABLE_COL].iloc[0]
                                    if AVAILABLE_COL in match.columns
                                    else budget_2025
                                )
                                ...
                                if consumed_now > 0:
                                    ...
                                    with col1a:
                                        st.write(
                                            f"**Total Consumed:** ${new_consumed_total:,.2f}"
                                        )
                                    with col2a:
                                        st.write(
                                    ...
                                    if available_after < 0:
                                        st.error(
                                            "⚠️ This expense will exceed the available budget!"
                                        )
                                    elif available_after < (budget_2025 * 0.1):
                                        st.warning("⚠️ Low budget remaining!")
                                form_data = {
                                    "selected_cc_number": selected_cc_number,
                                    "selected_cc_name": selected_cc_name,
                                ...
                                st.error(
                                    "❌ No exact match found for the selected combination."
                                )
                                consumed_now = 0
                                form_data = {}
                            submit = st.form_submit_button(
                                "📝 Log Expense", type="primary"
                            )
                        if (
                            submit
                            and "form_data" in locals()
                            and form_data
                            and consumed_now > 0
                        ):
                            try:
                                new_consumed_total = (
                                    form_data["consumed_before"]
                                    + form_data["consumed_now"]
                                )
                                available_after = (
                                    form_data["budget_2025"] - new_consumed_total
                                )
                                row = {
                                    "Cost Center Number": form_data[
                                        "selected_cc_number"
                                    ],
                                    "Cost Center Name": form_data["selected_cc_name"],
                                    "Account number": form_data["selected_acc_number"],
                                    "Account name": form_data["selected_acc_name"],
                                    "Date": form_data["expense_date"],
                                    "Quarter": get_quarter_from_date(
                                        form_data["expense_date"]
                                    ),
                                    "Year": form_data["expense_date"].year,
                                    "2023 Budget": form_data["budget_2023"],
                                    "2024 Budget": form_data["budget_2024"],
                                    "2025 Budget": form_data["budget_2025"],
                                ...
                                if success:
                                    st.success(
                                        "✅ Expense logged and saved successfully!"
                                    )
                                    st.balloons()
                                    st.rerun()
                                else:
                                    st.error(
                                        "❌ Failed to log the expense. Please try again."
                                    )
                            except Exception as e:
                                st.error(f"❌ Error logging expense: {str(e)}")
                        elif submit and consumed_now <= 0:
                            st.warning(
                                "⚠️ Please enter an expense amount greater than 0."
                            )
    ...
    with tab1:
        ...
        with col3:
            ...
            selected_ccs = st.multiselect(
                "Select Cost Centers", options=cc_options, default=["All"]
            )
        ...
        if "All" not in selected_accounts and selected_accounts:
            filtered_df = filtered_df[
                filtered_df["Account name"].isin(selected_accounts)
            ]
        st.markdown("---")
        if time_period == "Annual":
            if "All" not in selected_accounts and selected_accounts:
                ...
                account_summary = (
                    filtered_df.groupby(["Cost Center Name", "Account name"])[
                        [CONSUMED_COL, AVAILABLE_COL]
                    ]
                    .sum()
                    .reset_index()
                )
                melted_data = pd.melt(
                    account_summary,
                    id_vars=["Cost Center Name", "Account name"],
                    value_vars=[CONSUMED_COL, AVAILABLE_COL],
                    var_name="Budget Type",
                    value_name="Amount",
                )
                if not melted_data.empty:
                    fig_con_avail = px.bar(
                        melted_data,
                        x="Account name",
                        y="Amount",
                        color="Budget Type",
                        barmode="group",
                        title="Consumed vs. Available by Account",
                        color_discrete_map={
                            CONSUMED_COL: "#d62728",
                            AVAILABLE_COL: "#2ca02c",
                        },
                        text="Amount",
                        facet_col="Cost Center Name",
                        facet_col_wrap=4,
                    )
                    fig_con_avail.update_traces(
            ...
            if not selected_years:
                ...
                for year in selected_years:
                    ...
                    if (
                        budget_col in filtered_df.columns
                        and consumed_col in filtered_df.columns
                    ):
                        yearly = (
                            filtered_df.groupby("Cost Center Name")[
                                [budget_col, consumed_col]
                            ]
                            .sum()
                            .reset_index()
                        )
                        yearly["Year"] = year
                        yearly.rename(
                            columns={budget_col: "Budget", consumed_col: "Consumed"},
                            inplace=True,
                        )
                ...
                if annual_data:
                    annual_df = pd.concat(
                        [
                            pd.DataFrame([x]) if isinstance(x, dict) else x
                    ...
                    if not annual_df.empty and annual_df["Budget"].sum() > 0:
                        if not selected_ccs:
                            fig_annual = px.bar(
                                ...
                                x="Year",
                                y="Budget",
                                title="Total Annual Budget Comparison",
                                text="Budget",
                            )
                        ...
                        if not selected_ccs:
                            fig_annual_trend = px.line(
                                annual_df,
                                x="Year",
                                y="Budget",
                                title="Total Annual Budget Trend",
                                markers=True,
                            )
                        else:
                            fig_annual_trend = px.line(
                                ...
                                y="Budget",
                                color="Cost Center Name",
                                title="Annual Budget Trend by Cost Center",
                                markers=True,
                            )
                ...
                if selected_accounts:
                    ...
                    for year in selected_years:
                        ...
                        if budget_col and budget_col in filtered_df.columns:
                            year_data = (
                                filtered_df.groupby(
                                    ["Cost Center Name", "Account name"]
                                )[budget_col]
                                .sum()
                                .reset_index()
                            )
                            year_data.rename(
                                columns={budget_col: "Budget"}, inplace=True
                            )
                            year_data["Year"] = year
                    ...
                    if account_annual_data:
                        ...
                        if not account_df.empty and account_df["Budget"].sum() > 0:
                            fig_accounts = px.bar(
                                ...
                                color="Year",
                                barmode="group",
                                title="Annual Budget by Account",
                                facet_col="Cost Center Name",
                                facet_col_wrap=4,
                            )
            ...
            if not selected_quarters or selected_year is None:
                ...
                if budget_col and budget_col in filtered_df.columns:
                    ...
                    if not quarter_filtered.empty:
                        ...
                        quarter_filtered["Quarter"] = pd.Categorical(
                        ...
                        quarterly_consumed_agg = (
                            quarter_filtered.groupby(
                                ["Cost Center Name", "Account name", "Quarter"]
                            )[CONSUMED_COL]
                            .sum()
                            .reset_index()
                        )
                        ...
                        if (
                            not quarterly_consumed_agg.empty
                            and quarterly_consumed_agg[CONSUMED_COL].sum() > 0
                        ):
                            ...
                            st.plotly_chart(
                                fig_quarterly_trend, use_container_width=True
                            )

                            # --- Quarterly Consumed Amount Bar Chart ---
                            st.subheader(
                                f"📊 {selected_year} Consumed Amount by Quarter"
                            )
                            consumed_agg = (
                                quarter_filtered.groupby(
                                    ["Cost Center Name", "Account name", "Quarter"]
                            ...
                            if (
                                not consumed_agg.empty
                                and consumed_agg[CONSUMED_COL].sum() > 0
                            ):
                                ...
                                st.plotly_chart(
                                    fig_consumed_bar, use_container_width=True
                                )
                            else:
                                st.info(
                                    "No consumption data to display for the selected period."
                                )

                            # --- Consumed Amount by Cost Center Bar Chart ---
                            st.subheader(
                                f"📊 {selected_year} Consumed Amount by Cost Center"
                            )
                            consumed_by_cc = (
                                quarter_filtered.groupby("Cost Center Name")[
                                    CONSUMED_COL
                                ]
                                .sum()
                                .reset_index()
                            ...
                            if (
                                not consumed_by_cc.empty
                                and consumed_by_cc[CONSUMED_COL].sum() > 0
                            ):
                                ...
                                fig_consumed_cc_bar.update_traces(
                                    texttemplate="$%{text:,.2s}", textposition="outside"
                                )
                                st.plotly_chart(
                                    fig_consumed_cc_bar, use_container_width=True
                                )
                        else:
                            st.info(
                                "No budget data to display for the selected quarterly period."
                            )
                    else:
