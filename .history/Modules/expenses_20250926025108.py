...
def create_file_upload_section():
    ...
    # Always save/load from Data folder
    file_path = os.path.join("Data", "Budget Monitoring.xlsx")
    ...
    if uploaded_file is not None:
        try:
            ...
            # Save uploaded file to Data folder (overwrite existing)
    ...
    # 🔹 Auto-load if no upload but Data/Budget Monitoring.xlsx exists
    elif "uploaded_file" not in st.session_state and os.path.exists(file_path):
        try:
            df = pd.read_excel(file_path)
            if validate_excel_structure(df):
                st.session_state.uploaded_file = df
                st.info("ℹ️ Loaded existing 'Budget Monitoring.xlsx' from Data folder.")
            else:
    ...
    # If we reach here, return whether a valid DataFrame is stored
