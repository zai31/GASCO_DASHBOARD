...
def show_analysis_dashboard_dynamic_fixed(df, col_manager, cost_center_names, account_names):
    """Enhanced analysis dashboard with full dynamic year support - FIXED VERSION"""
    ...
    with col2:
        if time_period == "Annual":
            ...
            if col_manager.available_years:  # FIX: Add safety check
    ...
    # Account filter with safety check
    if "Account name" in filtered_df.columns and not filtered_df.empty:
    ...
    if time_period == "Annual" and selected_years:
        show_annual_analysis_dynamic_fixed(filtered_df, col_manager, selected_years)
    elif time_period == "Quarterly" and selected_quarters and col_manager.available_years:
        show_quarterly_analysis_dynamic_fixed(filtered_df, col_manager, selected_year, selected_quarters)
    ...
    if time_period == "Annual":
        ...
        if 'selected_year' in locals():  # Safety check
...
def show_annual_analysis_dynamic(df, col_manager, selected_years):
    """Show annual analysis with full dynamic year support - FIXED"""
    ...
    for year in selected_years:
        ...
        if "Cost Center Name" in df.columns and not df.empty:
...
def show_quarterly_analysis_dynamic(df, col_manager, selected_year, selected_quarters):
    """Show quarterly analysis with dynamic year support - FIXED"""
    ...
    # FIXED: Better data filtering with safety checks
    try:
        quarter_filtered = df[
        ...
        st.error(f"Error filtering quarterly data: {e}")
        return
    ...
    if not quarter_filtered.empty:
        ...
        if consumed_col and consumed_col in quarter_filtered.columns:
            agg_dict[consumed_col] = "sum"
        elif "Current_Consumed" in quarter_filtered.columns:
        ...
        try:
            ...
            with col3:
                if consumed_col in quarterly_summary.columns and total_budget > 0:
                    utilization = (total_consumed / total_budget * 100)
                    st.metric("Utilization %", f"{utilization:.1f}%")
                    
        except Exception as e:
            st.error(f"Error creating quarterly analysis: {e}")
    else:
        st.warning(f"No data found for {selected_year} in quarters {selected_quarters}")
...
def show_filtered_dashboard():
    ...
    show_filtered_dashboard_dynamic_f()
...
def show_budget_distribution_analysis(df, col_manager, year):
    """Fixed version of budget distribution analysis"""
    ...
    with col1:
        # FIXED: Cost center distribution with Pareto analysis
        ...
        fig1.add_trace(go.Bar(
            ...
            y=cc_budget.values,
            name="Budget",
            # REMOVED: yaxes parameter - doesn't exist
        ))
        fig1.add_trace(go.Scatter(
            x=cc_budget.index,
            y=cc_budget_pct.values,
            name="Cumulative %",
            yaxis="y2",  # FIXED: changed from yaxes to yaxis
            mode="lines+markers",
            line=dict(color="red")
        ))
        ...
        fig1.update_layout(
            ...
            xaxis=dict(tickangle=-45),  # FIXED: changed from xaxes_tickangle
            yaxis=dict(title="Budget Amount"),  # FIXED: changed from yaxes
            yaxis2=dict(title="Cumulative %", overlaying="y", side="right"),  # FIXED: changed from yaxes2
            height=400
        )
        st.plotly_chart(fig1, use_container_width=True)
...
def show_filtered_dashboard_dynamic():
    """Main dashboard function with complete dynamic year support - CORRECTED"""
    ...
    if col_manager.available_years:
        st.info(f"Available budget years: {', '.join(col_manager.available_years)}")
    else:
        st.warning("No budget years detected in the data")
        return
    ...
    # Enhanced Expense Logging Section (keep your existing code here)
    # ... (expense logging code remains the same)
    ...
    with tab1:
        show_analysis_dashboard_dynamic_fixed(df, col_manager, cost_center_names, account_names)
