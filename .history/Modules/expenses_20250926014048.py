...
# Fixed show_filtered_dashboard_dynamic function with proper structure
...
def show_filtered_dashboard_dynamic():
    ...
    # Enhanced Analytics Section - CORRECTED POSITION
    st.markdown("---")
    ...
    with tab2:
        show_summary_tab_dynamic(df, col_manager)
    
    with tab3:
        st.subheader("📈 Advanced Analytics")
        st.info("Advanced analytics features coming soon!")
        
        # Basic advanced analytics placeholder
        ...
        if current_budget_col and current_budget_col in df.columns:
            ...
            efficiency_df = df.groupby("Cost Center Name").agg({
                ...
                'Current_Consumed': "sum"
            }).reset_index()
            
            efficiency_df["Utilization (%)"] = (
                efficiency_df['Current_Consumed'] / efficiency_df[current_budget_col] * 100
            ).fillna(0)
            ...
            fig = px.bar(
                ...
                x="Cost Center Name",
                y="Utilization (%)",
                title=f"Budget Utilization by Cost Center ({col_manager.current_year_str})",
                color="Utilization (%)",
            ...
            with col3:
                low_efficiency = len(efficiency_df[efficiency_df["Utilization (%)"] < 50])
                st.metric("Low Efficiency Units", f"{low_efficiency}/{len(efficiency_df)}")
