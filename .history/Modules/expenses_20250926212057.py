...
def log_expense_form():
    ...
    with st.form("log_expense_form"):
        # Form fields
        cost_center = st.selectbox("Cost Center", options=["CC1", "CC2"])  # replace with dynamic options
        account = st.selectbox("Account", options=["Account1", "Account2"])  # dynamic
        ...
        if submitted:
            ...
            # Call your improved function
            success = improved_append_expense_to_excel(new_data)
            
            if success:
                st.success("Expense logged successfully!")
            else:
                st.error("Failed to log expense. Check console for errors.")
...
def show_budget_distribution_analysis(df, col_manager, year):
    ...
    with col1:
        ...
        fig1.update_layout(
            ...
            yaxis=dict(title="Budget Amount"),
